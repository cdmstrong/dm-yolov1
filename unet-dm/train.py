...
def Focal_Loss(inputs, target, cls_weights, num_classes=21, alpha=0.5, gamma=2):
    ...
    return loss
class Train():
    def __init__(self, loss, epochs = 100, batch_size = 4, learn_rate = 0.001) -> None:
        self.epochs = epochs
    ...
    def train(self):
        for epoch in range(self.epochs):
            for i,(img, label) in enumerate(self.data_loader):
                ...
                pred = self.model(img)
                loss = Focal_Loss(pred, label.squeeze(1), torch.ones([2]), 2)
                self.optimizer.zero_grad()
                loss.backward()
                self.optimizer.step()
                print("Epoch %d/%d| Step %d/%d| Loss: %.2f"%(epoch,self.epochs,i,len(self.dataset)//self.batch_size,loss))
            if (epoch+1)%10==0:
                torch.save(self.model,"./models_pkl/unet"+str(epoch+1)+".pkl")
